...
const experiences = [
    { title: "Software Developer Engineer", company: "Odoo", period: "Jul 2025 — Present", current: true, techStack: ["Javascript", "Python", "XML", "PSQL", "jQuery", "SCSS"], description: ["Contributing to the Website team, by improving the Odoo Website Builder module, optimizing page components, templates, and drag-and-drop features to improve user experience.", "Deepening understanding of web development gaining hands-on experience across both backend and frontend components."] },
        ...
        <section id="Experience" className="mt-10">
            ...
            <div className="flex flex-col gap-3">
                {experiences.map((exp, index) => {
                    ...
                    return (
                        <div key={index} onClick={() => setExpanded(isOpen ? null : index)} className={`cursor-pointer rounded-xl border transition-all duration-300 ${isOpen ? "bg-gray-50 dark:bg-neutral-900 border-gray-900 dark:border-white" : "bg-white dark:bg-neutral-950 border-gray-100 dark:border-neutral-800 hover:border-gray-300 dark:hover:border-neutral-600"}`}>
                            <div className="px-6 py-4">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between">
                                    ...
                                    <span className="text-xs text-app-muted font-mono shrink-0 sm:ml-4">{exp.period}</span>
                                </div>
                                <div className="grid transition-all duration-500 ease-in-out" style={{ gridTemplateRows: isOpen ? "1fr" : "0fr" }}>
                                    <div className="overflow-hidden">
                                        <div className="mt-4">
                                            <ul className="space-y-2 mb-4">
                                                {exp.description.map((point, i) => <li key={i} className="flex gap-2 text-sm text-app-muted leading-relaxed"><span className="mt-1.5 w-1 h-1 rounded-full bg-gray-300 dark:bg-neutral-600 shrink-0" />{point}</li>)}
                                            </ul>
                                            <div className="flex flex-wrap gap-1.5">
                                                {exp.techStack.map((tech, idx) => <span key={idx} className="px-2 py-0.5 text-xs font-medium bg-white dark:bg-neutral-800 text-app-secondary rounded-md border border-gray-200 dark:border-neutral-700">{tech}</span>)}
                                            </div>
                                        </div>
                                    </div>
                                </div>
