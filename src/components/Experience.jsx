        ...
        <section id="Experience" className="mt-10">
            ...
            <div className="divide-y divide-dashed divide-rule">
                {experiences.map((exp, index) => {
                    ...
                    return (
                        <div key={index}>
                            <button onClick={() => setExpanded(isOpen ? null : index)} aria-expanded={isOpen} className="w-full text-left cursor-pointer py-4">
                                <div className="flex items-center gap-2 mb-0.5">
                                    <p className="font-medium text-app-primary">{exp.company}</p>
                                    {exp.current && <span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-saffron opacity-75" /><span className="relative inline-flex rounded-full h-2 w-2 bg-saffron" /></span>}
                                </div>
                                ...
                                <div className="overflow-hidden">
                                    <div className="pb-4">
                                            ...
                                            {exp.description.map((point, i) => {
                                                ...
                                                return (
                                                    <li key={i} className="flex gap-2 text-sm text-app-muted leading-relaxed group/item">
                                                        <span className="mt-2.5 w-1 h-1 rounded-full bg-(--text-muted) shrink-0" />
                                                        {item.url ? (
                                                            <a href={item.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 hover:text-app-primary transition-colors">
                                                                {item.text}
                                                                <FiExternalLink className="opacity-60 group-hover/item:opacity-100 transition-opacity shrink-0" size={12} />
                                                            </a>
                                                        ) : item.text}
                                                    </li>
                                                );
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                            {exp.techStack.map((tech, idx) => <span key={idx} className="px-2 py-0.5 text-xs font-medium bg-surface text-app-secondary rounded-md border border-rule">{tech}</span>)}
                                        </div>
                                    </div>
                                </div>
                            </div>
