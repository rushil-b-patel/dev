  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
            vec2 planeAspect = uPlaneSizes / uPlaneSizes.y;
            vec2 imageAspect = uImageSizes / uImageSizes.y;

            vec2 scale = vec2(1.0);
            if ((imageAspect.x / imageAspect.y) > (planeAspect.x / planeAspect.y)) {
            scale.y = (planeAspect.x / planeAspect.y) * (imageAspect.y / imageAspect.x);
            } else {
            scale.x = (planeAspect.y / planeAspect.x) * (imageAspect.x / imageAspect.y);
            }

            vec2 centeredUV = vUv * 2.0 - 1.0; // convert [0,1] to [-1,1]
            centeredUV /= scale;
            if (abs(centeredUV.x) > 1.0 || abs(centeredUV.y) > 1.0) discard;
            vec2 finalUV = (centeredUV + 1.0) * 0.5; // back to [0,1]

            vec4 color = texture2D(tMap, finalUV);

            float d = roundedBoxSDF(finalUV - 0.5, vec2(0.5 - uBorderRadius), uBorderRadius);
            if(d > 0.0) {
                discard;
            }
