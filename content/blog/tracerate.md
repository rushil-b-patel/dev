...
Ping is TCP based, not ICMP. Reason being ICMP needs raw sockets which means root on macOS/Linux, and I didn't want users running my tool with sudo. So instead tracerate opens a TCP socket to `speed.cloudflare.com:443`, times how long the `connect()` takes, closes it, and repeats 5 times. From those 5 samples:
